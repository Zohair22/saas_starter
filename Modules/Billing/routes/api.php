...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware(['auth:sanctum', 'tenant', 'tenant.member', 'throttle:api', 'tenant.api.rate'])->group(function () {
        ...
        Route::post('billing/subscribe', [BillingController::class, 'store'])->middleware('mfa.stepup')->name('billing.subscribe');
        Route::patch('billing/subscription', [BillingController::class, 'update'])->middleware('mfa.stepup')->name('billing.swap');
        Route::delete('billing/subscription', [BillingController::class, 'destroy'])->middleware('mfa.stepup')->name('billing.cancel');
        ...
        Route::post('billing/payment-methods', [PaymentMethodController::class, 'store'])->middleware('mfa.stepup')->name('billing.payment-methods.store');
        Route::patch('billing/payment-methods/default', [PaymentMethodController::class, 'setDefault'])->middleware('mfa.stepup')->name('billing.payment-methods.default');
        Route::delete('billing/payment-methods/{paymentMethodId}', [PaymentMethodController::class, 'destroy'])->middleware('mfa.stepup')->name('billing.payment-methods.destroy');
    });
});
