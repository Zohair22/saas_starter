...
export default function TasksCreate({ projectId }) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            window.location.href = `/app/projects/${projectId}/tasks`;
        } catch (error) {
    ...
    return (
        <AppLayout title="Create Task" session={session}>
            <div className="mb-4">
                <Link
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-600">Capture the task clearly so collaborators can execute without back-and-forth.</p>
                    </div>
                    ...
                    <label className="block">
                        ...
                        <textarea
                            ...
                            className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none ring-slate-300 transition focus:border-slate-500 focus:ring"
                        />
                        <span className="mt-1 block text-xs text-slate-500">Include acceptance criteria and dependencies where possible.</span>
                        {errors.description ? (
                ...
                <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <p className="text-xs font-semibold tracking-wide text-slate-500 uppercase">Task creation tips</p>
                    <ul className="mt-2 space-y-2 text-sm text-slate-700">
                        <li>Keep each task outcome measurable.</li>
                        <li>Use medium priority as default unless urgency is real.</li>
                        <li>Set due dates to support planning, not pressure.</li>
                    </ul>
                </aside>
            </div>
        </AppLayout>
